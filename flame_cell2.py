...
def edge_cnt(input_img, ig_s):
    ...
    for i in range(len(contours)):
        cnt = contours[i]
        area = cv.contourArea(cnt)
        if area <= ig_s or area > 2000:
            continue
        cv.drawContours(dst_in, contours, i, (255, 255, 255), 1)

        contours_list.append(contours[i])

    dst_in = cv.resize(dst_in, (dst_in.shape[0], dst_in.shape[1]))
...
if __name__ == '__main__':
    ...
    """ Input Image"""
    background = cv.imread(path+file_p[1000])
    background = cv.cvtColor(background, cv.COLOR_BGR2GRAY)
    background = cv.morphologyEx(background, cv.MORPH_CLOSE, kernel_5)
    cv.imshow('bac', background)

    x_o, y_o, w_o, h_o = select_roi(background, 'bac')
    background = background[y_o:y_o + h_o, x_o:x_o + w_o]

    cur_frame = cv.imread(path+file_p[1000])                  # interface of the object picture######################
    ...
    binary_otsu = cv.dilate(binary_otsu, kernel_5)
    ...
    ks = cv.getStructuringElement(cv.MORPH_ELLIPSE, (3, 3))
    mask_adapt = cv.morphologyEx(mask_adapt, cv.MORPH_CLOSE, ks)
    contours_out, hierarchy_out = edge_cnt(mask_adapt, 3)

    """Cell information"""
    cell_area = cell_info(contours_out)[0]
    cell_length = cell_info(contours_out)[1]
    cell_equ_d = cell_info(contours_out)[2]
    cell_num = cell_info(contours_out)[3]
    ...
    with plt.style.context('ggplot'):
        ...
        ax.scatter(contours_points[:, :, 0].ravel(), contours_points[:, :, 1].ravel(), Z_3.ravel(), s=1, c='k')
        ax.axis('off')
        ...
        plt.plot(hist)
        fig4 = plt.figure()
        fig4 = plt.hist(cell_area, bins=int(max(cell_area)-min(cell_area)), density=1)
        plt.xlabel('area')
        plt.ylabel('Probability density')

        fig5 = plt.figure()
        fig5 = plt.hist(cell_length, bins=int(max(cell_length)-min(cell_length)), density=1)
        plt.xlabel('length')
        plt.ylabel('Probability density')
        plt.xticks([x for x in range(0, int(max(cell_length)), 10)])

    area_all = []
    for i_o in tqdm(range(len(contours_out))):
        ...
        for row in range(mask_board.shape[0]):
            for cols in range(mask_board.shape[1]):
                if mask_board[row][cols] == 255:
                    x_3d, y_3d = (cols - int(x_center), int(y_center) - row)
                    z_3d = c_3d * np.abs(1 - (x_3d ** 2) / (a_3d ** 2) - (y_3d ** 2) / (b_3d ** 2)) ** 0.5
                    area_sum = area_sum + np.abs(1+((c_3d**2)*(x_3d ** 2))/((a_3d ** 4)*(z_3d**2))+((c_3d**2)*(y_3d ** 2))/(
                            (b_3d ** 4)*(z_3d**2)))**0.5
    ...
    with plt.style.context('seaborn'):
        fig6 = plt.figure()
        plt.hist(area_all, bins=(int(max(area_all))), density=1)
        plt.xlabel('ThreeD RC Area')
